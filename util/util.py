...
class ProgressBar (object):
    def __init__ (self, text='unknown', maxval=0):
        self.bar = progressbar.ProgressBar (term_width=100, maxval=maxval, widgets=[
            ...
            progressbar.ETA ()])
        self.step = 2047
        if maxval < 2 ** 10 * 20: self.step = 127
        if maxval < 2 ** 10: self.step = 1
        print ('[\033[32m', text, '\033[0m:', maxval, ']')
        self.bar.start ()
    def update (self, progress):
        if progress & self.step == 0: self.bar.update (progress)
    def finish (self):
        self.bar.finish ()
...
class DirectoryWriter (dict):
    ...
    def __getitem__ (self, key):
        if self.openings > 128: self.close ()
        if key not in self or dict.__getitem__ (self, key) == None: 
