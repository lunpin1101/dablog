#!/usr/bin/python3
import copy, gc, gzip, json, numpy, os, progressbar, random, spacy, tensorflow
...
class Sequence (dablog.sequence.Sequence):
    ...
    def __getitem__ (self, idx, mode='autoencoder'):
        ...
        if mode == 'autoencoder': return self.getItemAutoencoder (idx)
...
class Model (object):
    ...
    def SetupGPU (gpulist = None):
        ...
        for i in range (0, len (lgpus)): tensorflow.config.experimental.set_memory_growth (lgpus [i], True)
    ...
    class Config (object):
        def __init__ (self, hidden_layer=2, hidden_unit=64, epochs=256, batch_size=2048, rank_threshold=0.05, 
            distance_threshold=0.05, use_repeat_vector=True, use_mimick_embedding=True, filepath=None, use_gzip=True, 
            verbose=False):
    ...
    def pretrain (self, trainset, mode='autoencoder'):
        ...
        if mode == 'autoencoder':
            ...
            nn_sizes = [None] + nn_sizes + nn_sizes [:: -1]
            for i in range (1, self.config.hidden_layer):
            ...
            else: nn.add (keras.layers.LSTM (nn_sizes [i], activation='relu', return_sequences=True, name='LSTM_' + str (i)))
    ...
    def load_mimick (self, filepath=None):
        ...
        self.mimick = dablog.mimick.Word2VecMimickEmbedding (0, 0, filepath=filepath, verbose=self.config.verbose)
        self.mimick.load ()
    ...
    def fit (self, trainset):
        if self.model is None:
            ...
            i = self.config.hidden_layer
            if self.config.use_repeat_vector:
                self.nn.add (keras.layers.LSTM (nn_sizes [i], activation='relu', return_sequences=False, name='LSTM_' + str (i)))
                self.nn.add (keras.layers.RepeatVector (self.seqlen))
            else: self.nn.add (keras.layers.LSTM (nn_sizes [i], activation='relu', return_sequences=True, name='LSTM_' + str (i)))
            for i in range (self.config.hidden_layer + 1, 2 * self.config.hidden_layer + 1):
                self.nn.add (keras.layers.LSTM (nn_sizes [i], activation='relu', return_sequences=True, name='LSTM_' + str (i)))
    ...
    def predict (self, testset):
        ...
        for seq in range (0, P.shape [0]):
            if self.config.verbose and seq & 1023 == 0: bar.update (seq)
            ...
            for evt in range (0, P.shape [1]):
                ...
                # conditions
                if rank > self.config.rank_threshold: append_to_result = True; break
                    ...
                    if len (dists) > 0 and min (dists) > self.config.distance_threshold: append_to_result = True; break
    ...
    def split (miss):
        ...
        for idx in range (0, len (X)):
            ...
            if x != y: ret.append ({'X': X, 'P': P [y_idx], 'I': I, 'idx': x_idx, 'x': x, 'y': y})
