...
def getDirectionFromLine (line, filepath):
    if line [srcip] == line [dstip]: return 'local'
    elif line [srcip] in filepath: return 'out'
    elif line [dstip] in filepath: return 'in'
    return 'na'

# keysets
#   to get stats, use this command: $ python3 ~/anom/mybin/unswnb15/key.py data/59.166.0.[1-9]/flows.csv data/149.171.126.[1-9]*/flows.csv
def getKeyFromLine (line, keyset=0, divisor=100):
    ...
    # key
    keys = [
        [proto, state, service, is_ftp_login, ct_ftp_cmd, ct_flw_http_mthd, trans_depth, is_sm_ips_ports, pktsum_idx],  # 706/366 keys when divisor is 100/1000 
...
def getLabelFromLine (line): 
    if line [label].strip () == '0': return None
    else: return line [attack_cat].strip ()
...
def main ():
    ...
    parser.add_argument ('-k', '--keyset', type=int, default=0)
    parser.add_argument ('--nodir', help='toggle off direction', action='store_true')
    ...
    for f in args.file:
        with open (f, 'rt') as fin:
            ...
            for linecount, line in enumerate (csvfin):
                if args.nodir: keystr = getKeyFromLine (line, args.keyset)
                else: keystr = getDirectionFromLine (line, f) + ',' + getKeyFromLine (line, args.keyset)
